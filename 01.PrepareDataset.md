...
### 数据集采样与切分策略说明（preprocess2_sampling_split.py）

本模块负责将原始的爬虫日志（归档/丢弃）转换为用于模型训练的标准数据集。为了避免模型过拟合单一来源并保证数据质量，我们采用了 **“宏观控比 + 微观加权”** 的组合策略。

#### 1. 采样核心逻辑

我们不仅仅是随机抽取，而是通过计算**采样权重 (Sampling Weights)** 来控制样本分布：

* **多样性保障 (Domain Balancing)**：
* **问题**：防止大流量网站（如门户网站）的数据淹没训练集。
* **算法**：**逆频率加权**。计算公式为 `1 / 该域名出现总次数`。
* **效果**：只有 1 条记录的小众博客与有 1000 条记录的门户网站，在采样时享有同等的曝光机会（平权）。


* **质量优选 (Quality Boosting)**：
* **问题**：保留低质量样本作为负例的同时，希望模型更多地学习高质量样本。
* **算法**：**分数线性加权**。基于 `APPENDIX_MAX_RATE_SCORE` 评分。
* **效果**：高分样本被选中的概率是低分样本的 **1.0 ~ 2.0 倍**。


#### 2. 数据集构成 (Configuration)

* **总量控制**：默认目标 **7,000** 条样本。
* **正负比 (Class Balance)**：
* **Dropped (负例)**：50% (旨在教会模型识别垃圾/无关内容)
* **Archived (正例)**：50% (旨在教会模型识别高价值内容)
* *注：如果某一类库存不足，会自动使用最大可用数量。*

#### 3. 切分策略 (Splitting)

* **比例**：**8 : 1 : 1** (训练集 : 测试集 : 验证集)。
* **方法**：**分层抽样 (Stratified Split)**。
* 确保训练集、测试集和验证集中，正负样本的比例始终保持一致（例如都是 1:1），避免因随机切分导致验证集出现偏差。
* *容错机制*：若某类样本极少导致无法分层，会自动回退到简单随机切分。

#### 4. 输入与输出

* **Input**: `summary_dropped.json` (负样本池), `summary_archived.json` (正样本池)
* **Output**: `dataset_train.json`, `dataset_test.json`, `dataset_val.json`
